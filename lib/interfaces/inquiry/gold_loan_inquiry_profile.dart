// ignore_for_file: library_private_types_in_public_api, use_build_context_synchronously, unused_field, avoid_print, unnecessary_null_comparison, prefer_const_constructors, curly_braces_in_flow_control_structures
...
class GoldLoanInquiryProForm extends StatefulWidget {
  final bool isInquiryPending;
  final String inquiryType;
  final String bearerToken;

  const GoldLoanInquiryProForm({
    Key? key,
  ...
  @override
  _GoldLoanInquiryProFormState createState() => _GoldLoanInquiryProFormState();
}

class _GoldLoanInquiryProFormState extends State<GoldLoanInquiryProForm> {
  ...
  Future<void> _loadCustomerProfile() async {
    try {
      CustomerProfile? customerProfile =
          await SharedPreferenceManager.getCustomerProfile();
  ...
  void _prefillFormFields() {
    if (_customerProfile != null) {
      setState(() {
        ...
        if (_customerProfile!.nic != null)
          nicController.text = _customerProfile!.nic!;
        if (_customerProfile!.address != null)
          addressController.text = _customerProfile!.address!;
        if (_customerProfile!.dob != null) {
          _dateController.text = DateFormat(
            'yyyy-MM-dd',
          ).format(_customerProfile!.dob!);
        }
        if (_customerProfile!.district != null) {
          selectedDistrict =
              DistrictManager.getDistrict(
                _customerProfile!.district!,
              )?.districtName;
        }
      });
    }
  ...
  Future<void> _uploadNicFiles(String token) async {
    try {
      if (_fileNameNicFront.isNotEmpty) {
        ...
        ResponseProfileNic responseNicFront = await _apiManager
            .uploadCustomerNic(
              token,
              _fileNicFront,
              _fileNameNicFront,
        ...
        if (!responseNicFront.error) {
          ...
          _showFlashMessage(
            context,
            "NIC front upload failed: ${responseNicFront.message}",
          );
        }
      }
      ...
      if (_fileNameNicBack.isNotEmpty) {
        setState(() => _isUploadingNic = true);
        ResponseProfileNic responseNicBack = await _apiManager
            .uploadCustomerNic(token, _fileNicBack, _fileNameNicBack, 'back');
        ...
        if (!responseNicBack.error) {
          ...
          _showFlashMessage(
            context,
            "NIC back upload failed: ${responseNicBack.message}",
          );
        }
      }
    } catch (e) {
    ...
    if (_formKey.currentState!.validate() && !_isSubmitting) {
      ...
      try {
        ...
        if (_fileNameNicFront.isEmpty || _fileNameNicBack.isEmpty) {
          _showFlashMessage(
            context,
            "Please upload both NIC front and back images.",
          );
          return;
        }
        ...
        CustomerProfile profile = CustomerProfile(
          ...
          district:
              selectedDistrict != null
                  ? districts
                      .firstWhere((d) => d.districtName == selectedDistrict)
                      .id
                  : null,
        ...
        ResponseProfileUpdate responseProfileUpdate = await _apiManager
            .updateCustomerProfile(token, profile);

        if (!responseProfileUpdate.error) {
          await _uploadNicFiles(token);

          final ResponseInquiryInitiated response = await _apiManager
              .initiateInquiry(token, widget.inquiryType);
  ...
  List<District> get districts =>
      DistrictManager.getDistricts().entries
  ...
  Widget buildNicUploadButton(bool isFront) {
    final hasFile =
        isFront ? _fileNameNicFront.isNotEmpty : _fileNameNicBack.isNotEmpty;
    ...
    return ElevatedButton(
      ...
      child: Text(
        hasFile ? fileName : "Upload ${isFront ? 'Front' : 'Back'} NIC",
      ),
    );
  }
